...
def expanded_coefficient_counts(length, is_zero):
  ...
  """
    ...
    if extra_needed < 0:
        raise ValueError('length={} cannot handle is_zero={}'.format(
            length, is_zero))
...
def expand_coefficients(coefficients, entropy, length=None):
  ...
  """
    ...
    coeffs_entropy = entropy * np.random.dirichlet(np.maximum(
        1e-9, counts - 1))
    ...
    for power in indices:
        coeffs = integers_with_sum(value=coefficients.item(power),
...
def sample_expanded_coefficients(degrees, entropy, length=None):
    ...
    coefficients = sample_coefficients(degrees,
                                       entropy / 2,
                                       max_non_zero=length)
...
def coefficients_to_polynomial(coefficients, variables):
    ...
    for power in indices:
        ...
        if (number.is_integer_or_rational(coeffs)
            ...
            raise ValueError('Unrecognized coeffs={} type={}'.format(
                coeffs, type(coeffs)))
        for coeff in coeffs:
...
def _sample_with_brackets(depth,
                          variables,
                          degrees,
                          entropy,
                          length,
    ...
    if random.choice([False, True]):
        ...
        while True:
            left = _sample_with_brackets(depth + 1, variables, degrees,
                                         entropy_left, length_left, True)
            right = _sample_with_brackets(depth + 1, variables, degrees,
                                          entropy_right, length_right, False)
            if random.choice([False, True]):
        ...
        def sample_with_zero_check(degrees_, entropy_, length_):
            while True:
                result = _sample_with_brackets(depth + 1, variables, degrees_,
                                               entropy_, length_, False)
        ...
        right = sample_with_zero_check(degrees_right, entropy_right,
                                       length_right)
...
def trim(coefficients):
    ...
    for axis in range(ndim):
        ...
        if slice_to < length:
            coefficients = coefficients.take(axis=axis,
                                             indices=list(range(slice_to)))
